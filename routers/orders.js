...
//GET request to return all details about an order to admin
router.get('/', async(req, res) => {
    //Get all orders with user column populaed with name and ordered by dateOrdered column
...
router.post('/', async (req, res) => {
    ...
    let newOrderItem = new OrderItem({
        quantity: req.body.orderItems.quantity,
        product: req.body.orderItems.product
    })

    newOrderItem = await newOrderItem.save();
    const orderItemsids = newOrderItem._id;
    ...
    if(!order) {
        res.status(404).send('Order Cannot be saved!')
    }
...
router.put('/:id', async (req, res) => {
    ...
    if(!order)    return res.status(400).send('order not processed');
    res.send(order);
})
